...
def draw_staff_lines(y, mn, mx, scale):
    ...
    def draw3line(y):
        x = XYPAPER + MARGIN
        editor.create_line(x, y, x+PRINTEAREA_WIDTH, y, width=2, capstyle='round',fill='black')
        editor.create_line(x, y+(10*scale), x+PRINTEAREA_WIDTH, y+(10*scale), width=2, capstyle='round',fill='black')
        editor.create_line(x, y+(20*scale), x+PRINTEAREA_WIDTH, y+(20*scale), width=2, capstyle='round',fill='black')


    def draw2line(y):
        x = XYPAPER + MARGIN
        editor.create_line(x, y, x+PRINTEAREA_WIDTH, y, width=0.5, capstyle='round',fill='black')
        editor.create_line(x, y+(10*scale), x+PRINTEAREA_WIDTH, y+(10*scale), width=0.5, capstyle='round',fill='black')


    def draw_dash2line(y):
        x = XYPAPER + MARGIN
        if platform.system() == 'Linux' or platform.system() == 'Darwin':
            editor.create_line(x, y, x+PRINTEAREA_WIDTH, y, width=1, dash=(6,6), capstyle='round',fill='black')
            editor.create_line(x, y+(10*scale), x+PRINTEAREA_WIDTH, y+(10*scale), width=1, dash=(6,6), capstyle='round',fill='black')
        elif platform.system() == 'Windows':
            editor.create_line(x, y, x+PRINTEAREA_WIDTH, y, width=1, dash=4, capstyle='round',fill='black')
            editor.create_line(x, y+(10*scale), x+PRINTEAREA_WIDTH, y+(10*scale), width=1, dash=4, capstyle='round',fill='black')
    ...
    def draw():
        ...
        def draw_barline_grid_stemwhite():
            ...
            for line in msg[pageno]:
                ...
                for bl in line:
                    ...
                    if bl[0] == 'gridline':
                        editor.create_line(event_x_pos(bl[1],l_counter,new_line),
                            curs_y,
                            event_x_pos(bl[1],l_counter,new_line),
                            curs_y+staffheight, dash=(6,6),fill='black')
                ...
                editor.create_line(XYPAPER+MARGIN+PRINTEAREA_WIDTH,curs_y,XYPAPER+MARGIN+PRINTEAREA_WIDTH,curs_y+staffheight,fill='black',width=2)
        ...
        def draw_header_and_titles():
            if pageno == 0:
                ...
                editor.create_text(XYPAPER+MARGIN, XYPAPER+MARGIN, text=title, anchor='w', font=("courier", 18, "normal"),fill='black')
                
                # draw composer on the first page
                editor.create_text(XYPAPER+MARGIN+PRINTEAREA_WIDTH, XYPAPER+MARGIN, text=composer, anchor='e', font=("courier", 12, "normal"),fill='black')
            ...
            editor.create_text(XYPAPER+MARGIN,curs_y+PAPER_HEIGHT-35,
                ...
                anchor='nw',
                font=("courier", 12, "normal"),fill='black')
                ...
                for note in line:
                    if render_type == 'normal':
                        if note[0] == 'note':
                            ...
                            editor.create_line(x1,y-(5*scale),x1,y+(5*scale),width=2*scale,fill='black')
                        if note[0] == 'split':
                            x0 = event_x_pos(note[1], l_counter, new_line)
                            x1 = event_x_pos(note[1]+note[2], l_counter, new_line)
                            y = note_y_pos(note[3], minnote, maxnote, curs_y, scale)
                        ...
                        if note[4] == 0: 
                            if note[3] in black:
                                ...
                                editor.create_oval(x,
                                    ...
                                    fill='white',
                                    outline='black')
                            editor.create_line(x,y-(25*scale),x,y,width=2*scale,fill='black')
                        if note[4] == 1: 
                            if note[3] in black:
                                editor.create_oval(x,
                                    y-(5*scale),
...
root.bind('<Motion>', write_or_edit_note)

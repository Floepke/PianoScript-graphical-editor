...
# editor --> leftpanel
editor = Canvas(rightpanel, bg='white', relief='flat',cursor='dot')
...
def my2pitch(my,y_scale):
    ...
    step = 5 * y_scale
    my -= XYPAPER
    step = round((((FULLSTAFFHEIGHT + 15) * y_scale) - my) / step)

    note = 0
    for c in range(step):

        if c in [4,9,16,21,28,33,40,45,52,57,64,69,76,81,88]:
            note -= 1
...
def draw(event):
    global length_of_music, file, y_scale
    ...
    # calc y_scale
    root.update()
    y_scale = editor.winfo_height() / (FULLSTAFFHEIGHT + (XYPAPER*2))
    print(editor.winfo_height())
    ...
    editor.configure(scrollregion=(0,0,bbox3+XYPAPER,bbox4+XYPAPER))
...
def grid_selector(event):
    global edit_grid
    value = ''
    for i in list_dur.curselection():
        value = list_dur.get(i)
    if '1' in value:
        edit_grid = (1024 / eval(divide_spin.get()))
    if '2' in value:
        edit_grid = (512 / eval(divide_spin.get()))
    if '4' in value:
        edit_grid = (256 / eval(divide_spin.get()))
    if '8' in value:
        edit_grid = (128 / eval(divide_spin.get()))
    if '16' in value:
        edit_grid = (64 / eval(divide_spin.get()))
    if '32' in value:
        edit_grid = (32 / eval(divide_spin.get()))
    if '64' in value:
        edit_grid = (16 / eval(divide_spin.get()))
    if '128' in value:
        edit_grid = (8 / eval(divide_spin.get()))

def lefthand(event='event'):
    
    global hand
    hand = 'left'

def righthand(event='event'):
...
handMenu = Menu(menubar, tearoff=0, bg='#333333', fg='white')
handMenu.add_command(label='up', command=righthand)
handMenu.add_command(label='down', command=lefthand)
menubar.add_cascade(label="stem", underline=0, menu=handMenu)
...
list_dur.bind('<Motion>', grid_selector)
divide_spin.bind('<Motion>', grid_selector)
panedmaster.bind('<ButtonRelease-1>', draw)

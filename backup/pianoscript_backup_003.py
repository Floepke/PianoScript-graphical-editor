...
# editor --> editorpanel
editor = Canvas(editorpanel, bg='white', relief='flat',cursor='pencil')
...
pview = Canvas(printpanel, bg='white', relief='flat',cursor='dot')
...
separator1 = ttk.Separator(rightpanel, orient='horizontal').pack(fill='x')
fill_label1 = Label(rightpanel, text='TITLES',bg=color1,fg='white',anchor='w')
fill_label1.pack(fill='x')
title_label = Label(rightpanel, text='Title: ',bg=color1,fg='white',anchor='w')
title_label.pack(fill='x')
title_entry = Entry(rightpanel)
title_entry.pack(fill='x')
composer_label = Label(rightpanel, text='Composer: ',bg=color1,fg='white',anchor='w')
composer_label.pack(fill='x')
composer_entry = Entry(rightpanel)
composer_entry.pack(fill='x')
copyright_label = Label(rightpanel, text='Copyright: ',bg=color1,fg='white',anchor='w')
copyright_label.pack(fill='x')
copyright_entry = Entry(rightpanel)
copyright_entry.pack(fill='x')

fill_label2 = Label(rightpanel, text='',bg=color1,fg='white',anchor='w')
fill_label2.pack(fill='x')
separator2 = ttk.Separator(rightpanel, orient='horizontal').pack(fill='x')
fill_label3 = Label(rightpanel, text='LAYOUT',bg=color1,fg='white',anchor='w')
fill_label3.pack(fill='x')
mpline_label = Label(rightpanel, text='Measures each line: ',bg=color1,fg='white',anchor='w')
mpline_label.pack(fill='x')
mpline_entry = Entry(rightpanel)
mpline_entry.pack(fill='x')
scale_label = Label(rightpanel, text='Global scale: ',bg=color1,fg='white',anchor='w')
scale_label.pack(fill='x')
scale_entry = Entry(rightpanel)
scale_entry.pack(fill='x')
margin_label = Label(rightpanel, text='Margin: ',bg=color1,fg='white',anchor='w')
margin_label.pack(fill='x')
margin_entry = Entry(rightpanel)
margin_entry.pack(fill='x')
system_label = Label(rightpanel, text='Space under system: ',bg=color1,fg='white',anchor='w')
system_label.pack(fill='x')
system_entry = Entry(rightpanel)
system_entry.pack(fill='x')
fill_label6 = Label(rightpanel, text='',bg=color1,fg='white',anchor='w')
fill_label6.pack(fill='x')

# fill_label4 = Label(leftpanel, text='',bg=color1,fg='white',anchor='w')
...
divide_spin = Spinbox(leftpanel, from_=1, to=100, bg='grey', increment=.5)
...
times_spin = Spinbox(leftpanel, from_=1, to=100, bg='grey', increment=.5)
...
apply_button = Button(rightpanel, text='apply grid mapping', anchor='w')
apply_button.pack(fill='x')
# fill_label7 = Label(leftpanel, text='t-sig amount grid-div visible',bg=color1,fg='white',anchor='w', font=("courier"))
# fill_label7.pack(fill='x')
# tsig_entry = Entry(leftpanel)
# tsig_entry.pack(fill='x')
# addgrid_button = Button(leftpanel, text='add item', anchor='w')
# addgrid_button.pack(fill='x')
# treecolumns = ('t-sig', 'amount', 'grid-div', 'visible')
# tree = ttk.Treeview(leftpanel, columns=treecolumns, show='headings', minwidth=10)
# tree.heading(treecolumns[0], text=treecolumns[0])
# tree.heading(treecolumns[1], text=treecolumns[1])
# tree.heading(treecolumns[2], text=treecolumns[2])
# tree.heading(treecolumns[3], text=treecolumns[3])
# treescrollbar = ttk.Scrollbar(tree, orient='v', command=tree.yview)
# tree.configure(yscroll=treescrollbar.set)
# tree.pack(fill='x')
# removegrid_button = Button(leftpanel, text='remove item', anchor='w')
# removegrid_button.pack(fill='x')
...
def draw_staff_lines(y, mn, mx, scale):
    ...
    def draw3line(y):
        ...
        pview.create_line(x, y+(10*scale), x+PRINTEAREA_WIDTH, y+(10*scale), width=2, capstyle='round',fill='black',tag='staff')
        pview.create_line(x, y+(20*scale), x+PRINTEAREA_WIDTH, y+(20*scale), width=2, capstyle='round',fill='black',tag='staff')


    def draw2line(y):
        x = MARGIN
        pview.create_line(x, y, x+PRINTEAREA_WIDTH, y, width=0.5, capstyle='round',fill='black',tag='staff')
        pview.create_line(x, y+(10*scale), x+PRINTEAREA_WIDTH, y+(10*scale), width=0.5, capstyle='round',fill='black',tag='staff')


    def draw_dash2line(y):
        x = MARGIN
        if platform.system() == 'Linux' or platform.system() == 'Darwin':
            pview.create_line(x, y, x+PRINTEAREA_WIDTH, y, width=1, dash=(6,6), capstyle='round',fill='black',tag='staff')
            pview.create_line(x, y+(10*scale), x+PRINTEAREA_WIDTH, y+(10*scale), width=1, dash=(6,6), capstyle='round',fill='black',tag='staff')
        elif platform.system() == 'Windows':
            pview.create_line(x, y, x+PRINTEAREA_WIDTH, y, width=1, dash=4, capstyle='round',fill='black',tag='staff')
            pview.create_line(x, y+(10*scale), x+PRINTEAREA_WIDTH, y+(10*scale), width=1, dash=4, capstyle='round',fill='black',tag='staff')
    ...
    if mn >= 1 and mn <= 3:
        ...
        draw3line(keyline+(170*scale)+y)
        draw2line(keyline+(210*scale)+y)
        editor.create_line(XYMARGIN + MARGIN, (keyline+(240*scale)+y), XYMARGIN + MARGIN + PRINTEAREA_WIDTH, (keyline+(240*scale)+y), width=2)
...
def run_pianroll(event='event'):
    ...
    if y_scale > 1.5:
        y_scale = 1.5
    center_pianoroll = (editor.winfo_height() - ((FULLSTAFFHEIGHT + (XYMARGIN * 2) + hbar.winfo_height()) * y_scale)) / 2
    x_scale = y_scale * .3
    ...
    # update grid map editor
    gridedit_text.delete('1.0', 'end')
    gridedit_text.insert('1.0', gridtext)
...
def mouse_handling(event, event_type):
    ...
    '''
    global btn1_click, btn2_click, btn3_click, holdid, new_note_id, cursor_note, cursor_time
    ...
    if event_type == 'btn1click':
        ...
        if taglst == () or taglst[0] in ['staff','current'] or taglst[1] == 'stem':
...
def keyboard_handling(event):
    ...
    global writing_note, cursor_time, cursor_note, selection, new_note_id
    print(event.keycode)
    ...
    if event.keysym == 'Left':
        ...
        # normal state
        if event.state == 16:
            cursor_time -= edit_grid

        # while holding shift
        if event.state == 17:

            # Shorten note length by one step
            for note in FILE[1]:

                if note['type'] == 'note':

                    if note['time'] == cursor_time and note['note'] == cursor_note:

                        note['duration'] -= edit_grid
                        if note['duration'] < edit_grid:
                            note['duration'] = edit_grid
    ...
    if event.keysym == 'Up':
        ...
        # normal state
        if event.state == 16:
            cursor_note = (cursor_note + 1) % 88

        # while holding shift
        if event.state == 17:
            
            # transpose up one step
            for note in FILE[1]:
    ...
    x = XYMARGIN + (cursor_time * x_scale)
    y = note_y_pos(cursor_note, 1, 88, XYMARGIN*y_scale, y_scale)
    ...
    for ts in t:
        ...
        if ts:
            try:
                ...
                print('Please read the documentation about how to provide the grid mapping correctly.\na correct gridmap:\n4/4 16 4 1')
                ignore = True
        ...
        # gridmap add to FILE
        FILE[0][0].append({'type':'time_signature', 'amount':amount, 'numerator':numerator, 'denominator':denominator, 'grid':grid_div, 'visible':visible})
    ...
    if platform.system() == 'Linux':
        ...
        if f:
            ...
            for rend in range(engrave('export')):
                pview.postscript(file=f"/tmp/tmp{rend}.ps", x=10000, y=rend*PAPER_HEIGHT, width=PAPER_WIDTH, height=PAPER_HEIGHT, rotate=False)
        ...
        if f:
            ...
            for export in range(engrave('export')):
                ...
                pview.postscript(file=f"{f.name}{counter}.ps", colormode='gray', x=10000, y=rend*PAPER_HEIGHT, width=PAPER_WIDTH, height=PAPER_HEIGHT, rotate=False)
                pslist.append(str('"'+str(f.name)+str(counter)+'.ps'+'"'))
            try:
                run_pianroll = subprocess.Popen(f'''"{windowsgsexe}" -dQUIET -dBATCH -dNOPAUSE -dFIXEDMEDIA -sPAPERSIZE=a4 -dEPSFitPage -sDEVICE=pdfwrite -sOutputFile="{f.name}.pdf" {' '.join(pslist)}''', shell=True)
                run_pianroll.wait()
...
def set_xscale(event='event'):
    ...
    value = simpledialog.askfloat('Set editor x-scale', 'x-scale is how big the x direction is zoomed. 0.4 = default; 0.8 = twice the default.')
...
def engrave(render_type=''):
    ...
    def read():
        ...
        msg = []
        count = 0
        for ln in new_line[:-2]:
            hlplst = []
            for evt in msgs:
                if evt[1] >= new_line[count]-0.1 and evt[1] < new_line[count+1]-0.1:
                    hlplst.append(evt)
            msg.append(hlplst)
            count += 1
        ...
        for line, height in zip(msgs, lineheight):
            icount += 1
            print(system_space)
            curs_y += height + int(system_space[0])# fix later
    ...
    def draw():
        ...
        for page, idx_page in zip(msg,range(len(msg))):
            ...
            for line, idx_line in zip(page,range(len(page))):
                ...
                for msgs, idx_msgs in zip(line,range(len(line))):
                    ...
                    if msgs[0] == 'barline':
                        x = event_x_pos_engrave(msgs[1],new_line[l_counter],new_line[l_counter+1])
                        pview.create_line(x,
                            curs_y,
                            x,
                            curs_y+staffheight,
                        ...
                        if msgs[4] == 0: 
                            if msgs[3] in BLACK:
                                pview.create_oval(x,
                                    ...
                                    fill='black',
                                    outline='black',
                                    tag=('note_start','black_note'))
                                if msgs[4] == 1:
                                    pview.create_oval(x0 + (2.5 * scale),
                                        y0 + (3.5 * scale),
                                        x0 + (5.5 * scale),
                                        y1 - (3.5 * scale),
                                        outline='',
                        ...
                        if msgs[4] == 1: 
                            if msgs[3] in BLACK:
                                pview.create_oval(x,
                                    ...
                                    y+(5*scale),
                                    width=2*scale,
                                    fill='black',outline='black',tag=('note_start','black_note'))
                            else:
                                pview.create_oval(x,
                                    y-(5*scale),
                                    x+(10*scale),
                            ...
                            pview.create_line(x,
                                ...
                                fill='black',
                                tag='note_start')
                            pview.create_oval(x + ((2.5-.5) * scale),
                                y0 + (3.5 * scale),
                                x + ((2.5+.5) * scale),
                                y1 - (3.5 * scale),
                                outline='',
                    ...
                    # note active
                    if msgs[0] == 'note' or msgs[0] == 'split':
                        x0 = event_x_pos_engrave(msgs[1],new_line[l_counter],new_line[l_counter+1])
                        x1 = event_x_pos_engrave(msgs[1]+msgs[2],new_line[l_counter],new_line[l_counter+1])
                        y = note_y_pos(msgs[3], minnote, maxnote, curs_y, scale)
                ...
                if len(msg)-1 == idx_page:
                    curs_y += staffheight + (int(sys_space))
                else:
                    curs_y += staffheight + (int(sys_space)) + (page_space[idx_page] / (len(msg[idx_page])))
        ...
        pview.tag_raise('tsig_text')
        pview.tag_raise('other')
        pview.tag_raise('white_space')
        pview.tag_raise('note_start')
        pview.tag_raise('connect_stem')
        pview.tag_raise('titles')
        pview.tag_raise('cursor')
        pview.tag_raise('endpaper')
        pview.tag_raise('black_note')
        pview.tag_raise('note_start_dot')
...
def set_global_scale():
    ...
    for i in FILE[0]:
        if isinstance(i,dict):
            if i['type'] == 'scale':
                i['value'] = simpledialog.askfloat('Set global scale', 'The global scale scales the drawing.\n 1=default, 2=twice as big.')
...
# MENU
#--------------------------------------------------------
menubar = Menu(root, relief='flat', bg=color1, fg='white')
...
setMenu = Menu(menubar, tearoff=0, bg=color1, fg='white')
...
menubar.add_cascade(label="Set", underline=0, menu=setMenu)

viewMenu = Menu(menubar, tearoff=0, bg=color1, fg='white')
viewMenu.add_command(label='reset pianoroll', command=run_pianroll)
...
apply_button.configure(command=process_grid_editor)

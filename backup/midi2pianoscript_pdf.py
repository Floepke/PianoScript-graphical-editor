...
if platform.system() == 'Windows':
    ...
    editor.bind("<MouseWheel>", _on_mousewheel)
    editor.bind("<Button-1>", zoomerP)
    editor.bind("<Button-3>", zoomerM)
...
separator2 = ttk.Separator(leftpanel, orient='horizontal').pack(fill='x')
pdf_button = Button(leftpanel, text='Export pdf...')
...
def event_x_pos(pos, linenr, newlinepos):
    ...
    pos -= newlinepos[linenr-1]
    return XYPAPER + MARGIN + pos * factor
...
def render(render_type, printpage=1):
        ...
        def draw_staff():
            ...
            for page in msg:
                ...
                for line in page:
                    ...
                    if len(msg) == p_counter:
                        curs_y += staffheight + (system_space)
                    else:
                        curs_y += staffheight + (system_space) + (page_space[p_counter-1] / (len(page)-1))
                    
                    # elif page_space[p_counter-1] < fillpagetreshold:
                    #     curs_y += staffheight + systemspacing + (page_space[p_counter-1] / (len(page)-1))
                    # elif page_space[p_counter-1] >= fillpagetreshold:
                    #     curs_y += staffheight + systemspacing

                curs_y = (PAPER_HEIGHT+XYPAPER) * p_counter + XYPAPER + MARGIN
        ...
        def draw_barline_grid_stemwhite():
            ...
            for page in msg:
                ...
                for line in page:
                    ...
                    # create the endline
                    if l_counter == lc:
                        editor.create_line(XYPAPER+MARGIN+PRINTEAREA_WIDTH,curs_y,XYPAPER+MARGIN+PRINTEAREA_WIDTH,curs_y+staffheight,width=5,capstyle='round')
        ...
        def draw_header_and_titles():
            ...
            for page in msg:
                ...
                editor.create_text(XYPAPER+MARGIN,curs_y+PAPER_HEIGHT-35,
                    text='page %s of %s | %s | %s' % (p_counter, len(msg), title, copyright),
                    anchor='nw',
                ...
                for line in page:
                    ...
                    for note in line:
                        ...
                        if note[0] == 'cursor' and render_type == 'normal':
                            x = event_x_pos(note[1], l_counter, new_line)
                            y = note_y_pos(note[3], minnote, maxnote, curs_y, scale)
                            editor.create_oval(x,y-(5*scale),x+(10*scale),y+(5*scale),fill='blue',outline='',tag='cursor')
...
def midi_import():
    ...
    # ---------------------------------------------
    try: midifile = filedialog.askopenfile(parent=root, 
        mode='Ur', 
...
pdf_button.configure(command=exportPDF)
...
# BIND (shortcuts)
#--------------------------------------------------------
# root.bind('<w>',move_cursor)
# root.bind('<s>',move_cursor)
# root.bind('<a>',move_cursor)
# root.bind('<d>',move_cursor)
#editor.bind('<1>',editor.focus)

...
HELP5 = '''Please provide two integers: 'insert-at-measure-number' and 'amount-of-measures-to-insert\''''
...
def new_file():
    global SaveFile, file_changed,file_path, TEMPLATE
    print('new_file')
        ...
        if f:
            ...
            for export in range(engrave('export')):
                ...
                pview.postscript(file=f"{f.name}{counter}.ps", 
                    colormode='gray', 
                    x=10000, 
                    y=export * (SaveFile['properties']['page-height'] * MM),
                    width=(SaveFile['properties']['page-width'] * MM), 
                    height=(SaveFile['properties']['page-height'] * MM), 
                    rotate=False,
                    fontmap='-*-Courier-Bold-R-Normal--*-120-*')
...
def add_measure():
    ...
    user_input = simpledialog.askstring('Transpose', HELP5)
...
toolsMenu.add_command(label='|add measure              |', command=lambda: add_measure())
...
'''
    ...
    * remove notes from measure x to y
'''
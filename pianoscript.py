...
fill_label6 = Label(rightpanel, text='time-signature|amount|grid-division|visible',bg=color1,fg='white',anchor='w', font=("courier"))
...
def text_design_rolleditor(txt):
    ...
    def round_rectangle(x1, y1, x2, y2, radius=5, **kwargs):
        ...
        return editor.create_polygon(points, **kwargs, smooth=True)
    ...
    t = editor.create_text(x,
        ...
        text=txt['text'],
        anchor='w')
    round_rectangle(editor.bbox(t)[0],
        editor.bbox(t)[1],
        editor.bbox(t)[2],
        editor.bbox(t)[3],
        fill='white',
...
def new_file():
    ...
    [
        ...
        {'type':'text', 'text':'test text', 'time':256, 'note':52, 'xoffset':0, 'yoffset':0}
    ]
    ]
...
def mouse_handling(event, event_type):
    ...
    '''
    global btn1_click, btn2_click, btn3_click, holdid, new_id, cursor_note, cursor_time
    ...
    if input_mode.get() == 'note':
        ...
        if event_type == 'btn1click':
            ...
            if taglst == () or taglst[0] in ['staff','current'] or taglst[1] == 'stem' or 'text' in taglst[0]:
            ...
            if 'text' in taglst:
                ...
                for txt in FILE[1]:
                    ...
                    if txt['type'] == 'text' and txt['id'] == holdid:
                        ...
                        txt['text'] = simpledialog.askstring('Edit text', 'Please edit your text here', initialvalue=txt['text'])
...
def midi_import():
    ...
    for i in mesgs:  
        if i['type'] == 'note_on' and i['channel'] == 0:
            FILE[1].append({'type':'note', 'time':i['time'], 'duration':i['duration'], 'note':i['note']-20, 'hand':'r', 'beam':0, 'slur':0})
        if i['type'] == 'note_on' and i['channel'] >= 1:
            FILE[1].append({'type':'note', 'time':i['time'], 'duration':i['duration'], 'note':i['note']-20, 'hand':'l', 'beam':0, 'slur':0})
    ...
    def read():
            ...
            if i['type'] == 'time_signature':
                ...
                for meas in range(0,i['amount']):
                    ...
                    for grid in range(0,i['grid']):
                        ...
                        grd_time += grid_len
                    bln_time += meas_len
                msg.append({'type':'time_signature_text', 'time':t_sig_start_tick(t_sig_map,count),'duration':meas_len,'text':str(i['numerator'])+'/'+str(i['denominator'])})
                count += 1
        ...
        for i in FILE[1]:
            ...
            if i['type'] == 'note':
                for note in note_split_processor(i):
                    msg.append({'type':note['type'], 'time':note['time'], 'duration':note['duration'], 'note':note['note'], 'hand':note['hand']})

            # invisible note
            if i['type'] == 'invis':
                msg.append({'type':note['type'], 'time':note['time'], 'duration':note['duration'], 'note':note['note']})
        ...
        for page in msg:
            ...
            for line in page:
                print('newline:')
                for note in line:
                    if note['type'] in ['note', 'split', 'invis', 'endline']:
                        print(note)
            ...
            for line, idx_line in zip(page,range(len(page))):
                ...
                for evt, idx_evt in zip(line,range(len(line))):
                    ...
                    if evt['type'] == 'note':
                        ...
                        x1 = event_x_pos_engrave(evt['time']+evt['duration'],new_line[l_counter],new_line[l_counter+1])
...
modeMenu.add_radiobutton(label="crescendo",variable=input_mode,value='cresc')
modeMenu.add_radiobutton(label="decrescendo",variable=input_mode,value='decresc')
input_mode.set('note')
menubar.add_cascade(label="Input_mode", underline=None, menu=modeMenu)
